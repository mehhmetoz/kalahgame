...
import com.bol.kalah.exception.GameException;

import static com.bol.kalah.model.Game.*;
...
public class GameEngine {
    ...
    public void play(int pitId) {
        ...
        if (isGameFinished())
            throw new GameException("GameEngine completed And Result is " + result);

        playerTurnRangeIn(pitId);

        int i;
        int seedCounts = game.getHouseValue(pitId);
        game.setHouseValue(pitId, 0);


        for (i = pitId + 1; seedCounts > 0; i = (i + 1) % TOTAL_HOUSES) {

            if (isWillPutOnOppositeHouse(i)) continue;
            game.addToHouseValue(i % TOTAL_HOUSES, 1);
            seedCounts--;

            if (isLastStoneInOwnEmptyPit(pitId, i, seedCounts)) {
                if (playerOneTurn) {
    ...
    private void playerTurnRangeIn(int pitId) {
